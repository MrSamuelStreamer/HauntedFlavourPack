...
// Gently lifted from https://github.com/feldoh/ScariaContaminationPatch/blob/main/1.5/Source/Comps/HediffComp_GeneGiver.cs
public class HediffComp_GeneGiver : HediffComp
{
    ...
    public override void CompPostMake()
    {
        ...
        if (pawn.genes != null && !Props.possibleGenes.All(g => pawn.genes.HasActiveGene(g)))
            return;
    ...
    public override void CompPostTick(ref float severityAdjustment)
    {
        ...
        if (!pawn.Spawned || (double)this.parent.Severity < 1)
            return;
        ...
        {
            Stack<GeneDef> viableGenes = new(Props.possibleGenes.Distinct().Except(pawn.genes.GenesListForReading.Select(g => g.def)).InRandomOrder());
        ...
        if (!PawnUtility.ShouldSendNotificationAbout(pawn))
            return;
        TaggedString label = "MSS_Haunted_LetterLabelGeneGiver".Translate();
        string letterText = Props.applyEntireXenotype ? "MSS_Haunted_LetterTextGeneGiverFullXenotype" : "MSS_Haunted_LetterTextGeneGiver";
